...
$R.add('service', 'cleaner', {
    init: function(app)
    {
        ...
        // regex
        this.reComments = new RegExp('<!--[\\s\\S]*?(?:-->)?<!---+>?|<!(?![dD][oO][cC][tT][yY][pP][eE]|\\[CDATA\\[)[^>]*>?|<[?][^>]*>?', 'g');
...
$R.add('service', 'marker', {
    ...
    restore: function()
    {
        ...
        {
            var $editor = this.editor.getElement();
            range = $editor.get().ownerDocument.createRange();
        }
...
$R.add('service', 'block', {
    ...
    _format: function()
    {
        ...
        else if (!this.selection.isCollapsed() && block && (block.tagName === 'TD' || block.tagName === 'TH'))
        {
            ...
            nextBr = $wrapper.get().nextSibling;
            if (nextBr && nextBr.tagName === 'BR')
            {
                $R.dom(nextBr).remove();
            }
...
$R.add('module', 'upload', {
    ...
    _sendData: function(data, e)
    {
        ...
        {
            this._complete(this.p.url(data, e));
        }
        else
        {
            $R.ajax.post({
...
$R.add('module', 'image', {
    ...
    _insert: function(response, e)
    {
        ...
        for (var key in response)
        {
            ...
            // does not have next sibling
            if (!$inserted[0].nextSibling)
            {
                this.utils.createMarkup($inserted[0]);
            }
...
$R.add('class', 'image.component', {
    ...
    _init: function(el)
    {
        ...
        var block = this.selection.getBlock();
        var isTable = ($R.dom(block).closest('table').length !== 0);
        ...
        {
            el = $el.children().not('span').first().get();
            $el = $R.dom(el);
            this.parse(el);
            this.$element = $el;
        }
        else if ($figure.length === 0 && $el.hasClass('redactor-component'))
        {
            this.parse(el);
            this.$element = $el;
        }
